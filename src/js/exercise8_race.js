...
require(['require', 'domReady', 'jquery', 'eig22', 'init/editor', 'init/grapher', 'init/viewer', 'init/edx_state', 'init/submission_guard', 'python/vm', 'python/simulation_proxy', 'python/internal_code', 'text!static/default.py'], function(require, domready) {
	domready(function() {
		...
		require('init/edx_state')({
			...
			'get_grade': function() {				
				return { 'time': beacons_all_active_time }
			},
			'tries': 5,
		});
		
		var beacons_active_count = 0;
		var beacons_all_active_time = -1;
		var beacons = [];
		
		// T
		var tx = 6.0, ty = 11.0;
		beacons.push(viewer.createBeacon([tx + 0.5, ty + 0, 1]));
		beacons.push(viewer.createBeacon([tx + 2.0, ty + 0, 1]));
		beacons.push(viewer.createBeacon([tx + 3.5, ty + 0, 1]));
		beacons.push(viewer.createBeacon([tx + 3.5, ty + 1.5, 1]));
		beacons.push(viewer.createBeacon([tx + 3.5, ty +-1.5, 1]));
		
		// U
		var ux = 3.5, uy = 7.0;
		beacons.push(viewer.createBeacon([ux + 0.5, uy + 1.5, 1]));
		beacons.push(viewer.createBeacon([ux + 2.0, uy + 1.5, 1]));
		beacons.push(viewer.createBeacon([ux + 3.5, uy + 1.5, 1]));
		beacons.push(viewer.createBeacon([ux + 0.5, uy + -1.5, 1]));
		beacons.push(viewer.createBeacon([ux + 2.0, uy + -1.5, 1]));
		beacons.push(viewer.createBeacon([ux + 3.5, uy + -1.5, 1]));
		beacons.push(viewer.createBeacon([ux + 0.5, uy + 0, 1]));
		
		// M
		var mx = 1.0, my = 2.0;
		beacons.push(viewer.createBeacon([mx + 0.5, my + 1.5, 1]));
		beacons.push(viewer.createBeacon([mx + 2.0, my + 1.5, 1]));
		beacons.push(viewer.createBeacon([mx + 3.5, my + 1.5, 1]));
		beacons.push(viewer.createBeacon([mx + 0.5, my + -1.5, 1]));
		beacons.push(viewer.createBeacon([mx + 2.0, my + -1.5, 1]));
		beacons.push(viewer.createBeacon([mx + 3.5, my + -1.5, 1]));
		beacons.push(viewer.createBeacon([mx + 2.5, my + 0, 1]));
		...
		plot_fcns = {
			...
			'marker': function(time, markers) {
			    //console.log(markers)
   				$.each(markers, function(id, marker) {
   					if(marker.length == 0) return;
   					
   					var m = marker[0];
       				viewer.createMarker([m[0], m[1]], m[2], id);
  				});
			},
		};
			...
			var success = simulation.run(code, function(time, data) {
				...
				$.each(beacons, function(idx, beacon) {
					if(!beacon.isActive() && beacon.distanceToDrone() < 0.5) {
						...
						if(beacons_active_count == beacons.length) {
							beacons_all_active_time = time;
						}
					}
				});
				
				if(beacons_all_active_time > 0) {
					simulation.stop();
				}
			}, function() {
				update_simulation_buttons(false);
